...
interface NewMachinesPageProps {
  initialFilters?: FilterParams;
  onNavigate: (view: string, id?: number) => void;
}

export const NewMachinesPage: React.FC<NewMachinesPageProps> = ({ initialFilters, onNavigate }) => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      {/* Top Banner */}
      <div className="relative rounded-3xl bg-gradient-to-r from-slate-900 via-slate-900 to-emerald-950/40 border border-emerald-500/30 p-8 overflow-hidden">
        <div className="relative z-10 max-w-2xl space-y-2">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-xs font-bold uppercase">
            ...
            Factory Fresh OEM Inventory
          </div>
          <h1 className="text-3xl sm:text-4xl font-black text-white">
            New Industrial Machines
          </h1>
          <p className="text-xs sm:text-sm text-slate-400">
            Brand new heavy machinery and automation systems direct from certified manufacturers and primary dealers with manufacturer warranties.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div className="hidden lg:block lg:col-span-1">
          <MachineFilters
            ...
            onChange={setFilters}
            onClear={() => setFilters({ listing_type: 'new', page: 1, page_size: 9, sort_by: 'newest' })}
          />
        </div>
        ...
        <div className="lg:col-span-3 space-y-6">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-slate-900/60 border border-slate-800 p-4 rounded-2xl">
            <div className="text-xs text-slate-400 font-medium">
              ...
              <strong className="text-amber-400">{total}</strong> new machines
            </div>
          ...
          {mobileFiltersOpen && (
            <div className="lg:hidden mb-6">
              <MachineFilters
                ...
                onClear={() => {
                  setFilters({ listing_type: 'new', page: 1, page_size: 9, sort_by: 'newest' });
                  setMobileFiltersOpen(false);
                }}
              />
            </div>
          ...
          {isLoading ? (
            ...
            <div className="text-center py-16 bg-slate-900/40 rounded-3xl border border-slate-800 p-8">
              <h3 className="text-lg font-bold text-white">No New Machines Found</h3>
              <p className="text-xs text-slate-400 mt-1 max-w-sm mx-auto">
                No new machinery matches your active filter criteria. Try clearing some filters or searching for different keywords.
              </p>
              <button
                onClick={() => setFilters({ listing_type: 'new', page: 1, page_size: 9, sort_by: 'newest' })}
